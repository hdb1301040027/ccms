...
/*************************
*@Description: 回调函数中完成Block_Data_1表中数据库的查询，并将多条记录发送至客户端
...
bool add(ccms_pro::ModuleData::Request &req,ccms_pro::ModuleData::Response &res)
{
    ...
    mysql_msg << "select TimeStamp,Module_Block_Voltage1,Module_Block_Voltage2,Module_Block_Voltage3,Module_Block_Voltage4 from Block_Data_1 \
	where ModuleID = "<< req.moduleID <<" and TimeStamp >= "<<req.StartStamp<<" and TimeStamp <= "<< req.EndStamp <<" ";
...
{
  ros::init(argc, argv, "history_block_data");
  ros::NodeHandle n;
  ros::ServiceServer service = n.advertiseService("history_block_msg",add);
  ROS_INFO("----Ready to select block database----");
  ros::spin();
  return 0;
}
