...
/*************************
*@Description: 回调函数中完成Error_Table表中数据库的查询，并将多条记录发送至客户端
...
bool add(ccms_pro::ModuleData::Request &req,ccms_pro::ModuleData::Response &res)
{
    ...
    mysql_msg << "select Error_Type_ID,PowerID,ModuleID,BlockID,StartTime,EndTime from Error_Table \
	where StartTime >= "<<req.StartStamp<<" and EndTime <= "<< req.EndStamp <<" ";
...
{
  ros::init(argc, argv, "history_error_data");
  ros::NodeHandle n;
  ros::ServiceServer service = n.advertiseService("history_error_msg",add);
  ROS_INFO("----Ready to select error database----");
  ros::spin();
  return 0;
}
